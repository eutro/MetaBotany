...
import eutros.metabotany.common.MetaBotany;
...
import eutros.metabotany.common.sound.MetaBotanySounds;
...
public class BindingLens extends ItemLens implements ICoordBoundItem {
    ...
    public static int colorOf(int colorId, int offset) {
        if(0 <= colorId && colorId <= 15) {
            ...
            return Color.HSBtoRGB((float) ((MetaBotany.proxy.getWorldElapsedTicks() * 2L % 360L) + offset) / 360.0F, 1.0F, 1.0F);
        }
    }
    ...
    protected boolean isBlacklist(ItemStack sourceLens, ItemStack compositeLens) {
        return compositeLens.getItem() == MetaBotanyItems.BOTANIA_FLARE_LENS;
    }
    ...
    public boolean collideBurst(IManaBurst burst, RayTraceResult traceResult, boolean isManaBlock, boolean dead, ItemStack stack) {
        if(!burst.isFake() && traceResult.getType() == RayTraceResult.Type.BLOCK) {
            ...
            if(!composite.isEmpty()) {
                ...
                if(item == MetaBotanyItems.BOTANIA_WARP_LENS) {
                    warp = true;
                } else if(item == MetaBotanyItems.BOTANIA_PAINT_LENS) {
                    paint = true;
                }
            }
            ...
            if(pos.equals(sourcePos) || (warp && block == MetaBotanyBlocks.BOTANIA_PISTON_RELAY)) {
                return super.collideBurst(burst, traceResult, isManaBlock, dead, stack);
            }
            ...
            PlayerEntity player = entity instanceof PlayerEntity ?
                                  (PlayerEntity) entity :
                                  new MetaBotanyFakePlayer((ServerWorld) world);

            if(simBind(world, stack, pos, block, side, player)) {
                if(!sourcePos.equals(UNBOUND_POS)) {
                    world.playSound(null, sourcePos, MetaBotanySounds.BOTANIA_DING, SoundCategory.BLOCKS, 0.5F, 1F);
                    TileEntity tileEntity = world.getTileEntity(sourcePos);
                    if(tileEntity != null)
                        tileEntity.markDirty();
                } else if(!(player instanceof FakePlayer)) {
                    world.playSound(null, player.getPosition(), MetaBotanySounds.BOTANIA_DING, SoundCategory.PLAYERS, 0.5F, 1F);
                }
            }
        }
        return super.collideBurst(burst, traceResult, isManaBlock, dead, stack);
    ...
    private boolean simBind(World world, ItemStack stack, BlockPos pos, Block block, Direction side, PlayerEntity player) {
        ...
        if(block instanceof IWandable) {
            ...
            if((tile instanceof IWandBindable && ((IWandBindable) tile).canSelect(player, stack, pos, side))
                    || block == MetaBotanyBlocks.BOTANIA_PISTON_RELAY) {
    ...
    private boolean tryCompletePistonRelayBinding(World world, BlockPos src, BlockPos pos) {
        if(src.equals(pos))
            return false;
        if(world.getBlockState(src).getBlock() == MetaBotanyBlocks.BOTANIA_PISTON_RELAY) {
    ...
    public ActionResult<ItemStack> onItemRightClick(@NotNull World world, PlayerEntity player, @Nonnull Hand hand) {
        ...
        if(player.isShiftKeyDown()) {
            if(!world.isRemote)
                setBindingAttempt(stack, UNBOUND_POS);
            player.playSound(MetaBotanySounds.BOTANIA_DING, 0.1F, 1F);
